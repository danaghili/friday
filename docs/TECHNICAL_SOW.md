...
- INC-100 — Ship friday public: a fresh clean marketplace repo, MIT-licensed, the lab kept private (approved 2026-07-25)
...
- INC-202 — The proposal ledger becomes a pipeline: numbered stages, a live header over a frozen ask, and a close that moves its own source (approved 2026-07-30)
- INC-203 — Stop writing facts that can rot: prevention at the authoring source, one probe, and the machinery parked until the evidence asks for it (approved 2026-07-30)
- INC-204 — Secrets get a home before they get a leak: the store declared at setup, a check that never opens a value file, and the scanners lifted from projects that already run them (approved 2026-07-30)
- INC-205 — The build lead's progress list stops being a habit: created before the work, kept true as it lands, recoverable after a crash (approved 2026-07-30)
- INC-206 — Old decisions stop being blockers: launch-era stances become dated context, every reversal gets written down, and the record gets corrected when one lands (approved 2026-07-30)
- INC-207 — The no-lie guarantee reaches the front end: JavaScript and TypeScript structure extracted deterministically, and the documents made maintainable at real project size (approved 2026-07-31)
- INC-208 — Lanes stop re-deriving their briefings: one shared template, the first real bundled file, and a checker that reads what was actually sent (approved 2026-07-31)
- INC-209 — Every scaffolded project gets the compaction guardrail: a standard seed, a narrow way into files friday has already handed over, and a check that reads what is actually there (approved 2026-07-31)
- INC-101 — The document check reaches the projects that need it: one checker that travels, a handover that cannot skip the read, and the parked remainder sent to its real home (approved 2026-07-31)
- INC-102 — Operations stops being judged and starts being proven: an enumerable battery, drills that expire when the system moves, and a job list nobody has to remember (approved 2026-07-31)
- INC-103 — Nobody watched after the pick: the coverage is counted from the project rather than claimed, the check that already existed gets a name, and the machine that opens updates is not handed to a project where a merge deploys (approved 2026-07-31)
- INC-104 — The impact analysis stops being believed: the question turns around at the moment of change, the person is asked what no file records, and "no change required" becomes a claim with a test attached (approved 2026-08-01)
- INC-105 — The rules a project writes down start being counted: declared conventions given checks that persist, a baseline every live service needs, and a sweep that reports without ever blocking (approved 2026-08-02)
- INC-106 — What a person sees when it breaks gets a reviewer: the designed states become an oracle after the build, comparable paths are compared against each other, and a state that exists but does nothing stops counting as one (approved 2026-08-02)
- INC-107 — A decision made in a code comment stops being invisible: the loose-deferral scan, a home test that reads rather than matches, and an answer that never has to be given twice (approved 2026-08-02)
- INC-108 — Sensitive data stops being a comment nobody acts on: a closed treatment set, answers that become requirements, and a declaration the deep clean reads back (approved 2026-08-02)
- INC-109 — Time stops being the only proxy for rot: an event arm beside the clock, counted from the records the project already commits (approved 2026-08-02)
- INC-110 — A sentence stops meaning different things to different readers: the machine named instead of pointed at, one rule inside a sweep that already runs, and the standing false sentences cleared (approved 2026-08-02)
