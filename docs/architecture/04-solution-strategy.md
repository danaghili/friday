...
Heavy front (TSOW authored under the grilling protocol, approved once) ·
...
- **K-model state (A.3, resolves OQ-1)** — project-level, precision-first,
  ...
  gate.
- **Version 0.5.0 replace-in-place** — humility before the DoD gate; rename
  costs are lockstep-wide `[D-0002]`.
