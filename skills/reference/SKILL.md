...
- **Prose discipline binds every section you write** (contract: `docs/contracts/synthesis-handoff.md` § "Prose rules binding the synthesis" — the rules' single home is the project `CLAUDE.md` § Conventions, D-0161): no derivable figure in prose — a count or list the IR or a tool owns is written as "every …" or as a pointer to its generated source, never as the figure; no restated single-homed rule — point at its home; no reader-relative referent carrying a machine fact — name the machine's committed anchor or the setting the tools read, never a phrase that rebinds to whoever opens the file. The oracle reads no prose (§ "What the oracle cannot do"), so an oracle-clean verdict never certifies a sentence — these rules are what stands between a regeneration and a confidently false claim.
