...
- `python3 "${CLAUDE_PLUGIN_ROOT}/tools/secret_posture_check.py" --root . --json` — the secret-store posture (INC-204): value-blind by construction, reporting here like the rest of the set; its one bite is the keys completion gate in §5 (D4).
...
The package is assembled from records and experts friday already has — never invented. Read: the intake brief (`docs/contracts/intake-brief.md` — the ownership/keys and client-tier source), the reference set (`docs/architecture/` arc42), `docs/DECISIONS.md` (the "why", to be rewritten client-plain), and the requirement-coverage ledger (the "promised and proven" list). Compose every briefing from `${CLAUDE_PLUGIN_ROOT}/docs/dispatch-briefing-template.md` and save it at dispatch with `--prompt-file` — from the file, never from memory: a reassembled briefing drops pieces silently, and every briefing on disk before INC-208 was missing one. Spawn the two operations experts (models named; telemetry via `spawn_telemetry.py`; each spawn message carries the `friday-docs: available` stamp — or a plain-Read pointer to `${CLAUDE_PLUGIN_ROOT}/docs/teammate-contract.md` — and the explicit Read list):

- **`friday-operations`** (model: **sonnet**, `--phase handoff:ops`): a **short, practical runbook — "when X breaks, do Y", never a fat incident manual** — plus the **operations battery's current verdicts** (the row set and verdict grammar live in `docs/contracts/ops-battery.md` — the battery's single home, cited never restated; INC-102 D8): the `restore` row's **proven** verdict is the evidence the restore gate needs, and the maintenance schedule carries each row's verdict and date (the table's shape stays owned by `docs/contracts/handoff-package.md`).
...
The gates: **reconcile** (satisfied only by a deep clean whose run included the document-truth probe, or by the PM's explicit recorded decision to hand over with findings outstanding — the attest tool refuses a bare confirmation and requires `--note` pointing at one or the other; gate meaning stated in `docs/contracts/handoff-package.md` § Completion gates, INC-101 — §1's mechanical verifier set is unchanged and was never this gate's satisfier: none of it reads a sentence against the code); **keys** (the PM attests every account/key moved into the client's name inside their secrets manager, builder access + recovery removed — friday never sees the values; **the attest tool refuses while value-carrying files are tracked in git and no accepted-risk record exists** — INC-204/D4, the increment's single bite, and the same precondition governs a repository visibility flip: a repo does not go public in that state); **restore** (a successful test restore evidenced, from §2's operations consult); **receiver** (a named client-side person has acknowledged the package). Never report done with any gate outstanding; show exactly what remains.
