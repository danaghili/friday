...
- **Secret store (INC-204):** where this project's secret values live. Derive the confirmed stack's native answer and propose it back — a platform's environment store, an orchestrator's secrets, Home Assistant's `secrets.yaml` — never a cold ask, and never a menu where the stack has an obvious native store; where the PM's own preference applies, name Infisical and its run-time injection pattern concretely in the instructions (D9 — instructions only, friday gains no adapter and no dependency). The repository carries names; values live in the store. Declining is allowed and recorded as an accepted risk with the PM's reason (support, never force — INC-204 D3), never a blank and never a silence.
- **Merge-reaches-production (INC-103):** part of this question set, but it fires at §7a's watcher seed rather than here — its recorded answer gates that one write, and its spelling's single home is `docs/contracts/claude-scaffold.md` § The dependency-update seeds.
...
| Path | Carries |
|---|---|
| `CLAUDE.md` | Stack + conventions + domain summary · the reader-relative authoring rule seeded into the conventions section (text owned by `docs/contracts/claude-scaffold.md` § The authoring-rule seed — cited, never restated; INC-110 D4, landing spot D-1066) · the FRIDAY-CLAIMS block (below) · exposure/deployment profile · environments (seeded under the exact heading `## Environments & deployment` — the section operations maintains after bootstrap, `agents/roles/operations.md`) · **scale profile** · the `FRIDAY-SECRET-STORE` block carrying Step 2's secret-store answer (its own additive block, never a FRIDAY-CLAIMS line — the declaration is not mechanically verifiable; grammar and empty case: D-0167, checker: `tools/secret_posture_check.py`; a decline lands as its `accepted-risk — <reason>` line, never a blank) · the `merge-deploys:` typed line, written at §7a's watcher-seed question and only then (spelling's single home: `docs/contracts/claude-scaffold.md` § The dependency-update seeds; `unknown` first-class, absent = never asked) · reuse-catalog pointer · the FRIDAY-STATE block (below). The always-loaded index — self-contained for its summaries. |
...
### 7b. Seed the maintainability bars (INC-008)
...
- **Seed the don't-read rule** per the contract's own section (INC-204, D11): globs derived from the confirmed stack's value-file names — not from project structure — resolved live like every other glob and quoted in the DONE relay; example files excluded by name. No value file on the real tree → no rule, and the relay says so.
- **Seed the compaction guardrail** per the contract's § The compaction guardrail. Do not hand-write the values: once your `settings.json` is on disk, run `python3 "${CLAUDE_PLUGIN_ROOT}/tools/compaction_seed.py" --apply --root . --greenfield` and quote its output. The tool reads the pair out of the contract, so the contract stays the one place either number lives and you never carry a copy that can drift. `--greenfield` is the init authority — friday is writing this file at this moment — and is a different thing from the `--consented` retrofit exception; neither flag ever stands in for the other. Then **announce it in your DONE relay**, in the PM's own terms, carrying exactly four things: what it does (this project's sessions tidy up their own context early, so friday's continuity work has room to run instead of firing when the context is already nearly full); what it assumes (a large-window model — friday cannot read which model a session runs and it can change between sessions, so this is **stated, never detected**); that it is a **shared project default** which an individual may override in their own `settings.local.json`, a file friday never writes; and **how to remove it permanently** — delete the pair from the file, which is the project's from the moment it is written, and friday never re-applies a seed the PM removed. Point the PM at the file they can open rather than quoting values into the relay (INC-209, D1, D6).
...
- **The delivery-configuration seed rides here** when the Stage-0 findings in your spawn message show a **GitHub origin remote**: write `.github/workflows/secret-scan.yml` copied exactly from the contract's template — same add-only rules; a project whose workflows already scan for secrets is skipped and reported. No GitHub remote → seed nothing; the delivery work lifts it from the contract when it wires CI (INC-204).
- **The dependency-update seeds ride the same condition** (contract § The dependency-update seeds — INC-103): the advisory-scan workflow copied exactly from the contract's template, skipped and reported where any workflow already runs an advisory scan; and the update watcher `.github/dependabot.yml` with its entries GENERATED from `python3 "${CLAUDE_PLUGIN_ROOT}/tools/watcher_coverage.py" kinds --root . --json` — the derivation's output quoted in your completion relay as the evidence, never a prose count — but the watcher file is written ONLY after the merge-reaches-production question has been asked as a `QUESTION_PAYLOAD` through the lead and the answer recorded as the project `CLAUDE.md`'s `merge-deploys:` typed line (spelling and the unknown-vs-absent rule: contract § The dependency-update seeds; where the operations battery's restart row already holds a verdict — `python3 "${CLAUDE_PLUGIN_ROOT}/tools/ops_battery.py" read --root .` — that verdict is the answer's source and you do not re-ask).: on a yes or unknown, state plainly that accepting a routine update on this project is a deployment, and proceed on the PM's word — you never refuse, you never decide, and you never write this file as a side effect of the rest of the seed set. An existing watcher configuration is skipped, reported, and never edited; then run `python3 "${CLAUDE_PLUGIN_ROOT}/tools/watcher_coverage.py" compare --root .` against what is already there and carry its output in the DONE relay.
...
- Authoring-rule seed: seeded into the CLAUDE.md conventions section (contract § The authoring-rule seed)
...
- Secret-scan workflow: {seeded / skipped — existing scan reported / not seeded — no GitHub remote} (contract § The delivery-configuration seed)
- Update watcher: {seeded — kinds derivation quoted above / skipped — existing config reported, compare output quoted above / not seeded — no GitHub remote}; merge-deploys: {yes / no / unknown — recorded in CLAUDE.md, or read from the battery's restart verdict}
- Advisory-scan workflow: {seeded / skipped — existing advisory scan reported / not seeded — no GitHub remote} (contract § The dependency-update seeds)
